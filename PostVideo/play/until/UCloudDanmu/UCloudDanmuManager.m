...
@implementation UCloudDanmuManager
...
- (void)p_initData
{
    ...
    self.danmuView = nil;
    self.danmuView = [[UCloudDanmuView alloc] initWithFrame:_frame];
...
- (void)p_danmu:(NSTimeInterval)startTime
{
    ...
    {
        UCloudDanmu *obj = [self.infos objectAtIndex:i];
        CGFloat time = obj.time;
    ...
    if (danmuInfos.count > 0)
    {
        ...
        void(^func)(UCloudDanmuLabel *danmuLabel, NSUInteger idx, CGFloat offsetXY) = ^(UCloudDanmuLabel *danmuLabel, NSUInteger idx, CGFloat offsetXY){
            if (idx != NSNotFound)
            {
        ...
        [danmuInfos enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL *stop) {
            __block UCloudDanmuLabel *danmuLabel = [UCloudDanmuLabel createWithInfo:(UCloudDanmu *)obj inView:weakSelf.danmuView];
            if ([danmuLabel isMoveModeFadeOut]) {
                [weakSelf p_getFadeBestChannel:danmuLabel completion:^(NSUInteger idx1, CGFloat offsetY) {
                    func(danmuLabel, idx1, offsetY);
                }];
            }
            else {
                [weakSelf p_getRollBestChannel:danmuLabel completion:^(NSUInteger idx2, CGFloat offsetX) {
                    func(danmuLabel, idx2, offsetX);
                }];
            }
        }];
    }
...
- (void)p_getRollBestChannel:(UCloudDanmuLabel *)newDanmuL completion:(void(^)(NSUInteger idx, CGFloat offsetX))completion
{
    ...
    {
        if (i < self.arRollChannelInfo.count)
        {
            id obj = [self.arRollChannelInfo objectAtIndex:i];
            index = i;
            if ([obj isKindOfClass:[UCloudDanmuLabel class]])
            {
    ...
    if (bFind)
    {
        ...
        if ([obj isKindOfClass:[UCloudDanmuLabel class]])
        {
            CGFloat x = ((UCloudDanmuLabel *)obj).currentRightX;
    ...
    else
    {
        ...
        else
        {
            ...
            index1 = [self p_allChannelWithPosition:danmuPosition new:newDanmuL];
            if (index1 != NSNotFound)
            {
                UCloudDanmuLabel *obj = [self.arRollChannelInfo objectAtIndex:index];
                [self.arRollChannelInfo replaceObjectAtIndex:index withObject:newDanmuL];
...
//选择完全不会碰撞的航道
- (BOOL)p_last:(UCloudDanmuLabel *)lastDanmuL new:(UCloudDanmuLabel *)newDanmuL
{
...
//选择在不超出缓冲区的且缓冲区最长的航道
- (NSUInteger)p_allChannelWithPosition:(DanmuPositionStruct)danmuPosition new:(UCloudDanmuLabel *)newDanmuL
{
    ...
    for (int i = (int)danmuPosition.start; i < danmuPosition.start + danmuPosition.length; i++)
    {
        ...
        if ([obj isKindOfClass:[UCloudDanmuLabel class]])
        {
            CGFloat rightX = ((UCloudDanmuLabel *)obj).currentRightX;
...
- (void)p_getFadeBestChannel:(UCloudDanmuLabel *)newDanmuL completion:(void(^)(NSUInteger idx, CGFloat offsetY))completion
{
...
- (NSUInteger)p_arDanmuLabel:(NSMutableArray *)arDanmuLs position:(DanmuPositionStruct)danmuPosition new:(UCloudDanmuLabel *)newDanmuL
{
    BOOL bFind = NO;
    NSUInteger index = danmuPosition.start;
    for (int i = (int)danmuPosition.start; i < danmuPosition.start + danmuPosition.length; i++)
    {
        ...
        if ([obj isKindOfClass:[UCloudDanmuLabel class]])
        {
            UCloudDanmuLabel *lastDanmuL = obj;
            CGFloat durationTime = newDanmuL.startTime - lastDanmuL.startTime;
...
- (void)initStart
{
    if (_danmuManagerState == DanmuManagerStateWait || _danmuManagerState == DanmuManagerStateStop)
    {
...
- (void)resume:(NSTimeInterval)nowTime
{
    ...
    dispatch_sync(self.danmuQueue, ^{
        ...
        {
            if ([subview isKindOfClass:[UCloudDanmuLabel class]])
            {
                [(UCloudDanmuLabel *)subview resume:nowTime];
            }
        }
    });
}
...
- (void)insertDanmu:(UCloudDanmu *)info
{
    dispatch_sync(self.danmuQueue, ^{
        __block UCloudDanmuLabel *danmuLabel = [UCloudDanmuLabel createWithInfo:info inView:self.danmuView];
        if ([danmuLabel isMoveModeFadeOut])
        {
